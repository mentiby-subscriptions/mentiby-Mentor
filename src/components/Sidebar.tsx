...
export default function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  ...
  const displayName = mounted && user
    ? (user.name || user.email?.split('@')[0] || 'Mentor')
    : 'Mentor'
      ...
      <div className="p-4 sm:p-6 border-b border-border/50">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">Mentor Dashboard</p>
          </div>
        </div>
      </div>
            ...
            <div className="absolute bottom-full left-0 right-0 mb-2 bg-card/80 backdrop-blur-xl border border-border/50 rounded-xl shadow-2xl z-50">
              ...
              <div className="p-3 sm:p-4 border-b border-border/50">
                ...
                <p className="text-xs text-muted-foreground">
                  Mentor ID: {mounted && user?.mentorId ? user.mentorId : '—'}
                </p>
              </div>
